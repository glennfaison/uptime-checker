...
tokenController.post = async (req, res) => {
  ...
  if (!created) {
    return res.json({ error: "Could not create new token" }, 500);
  }
...
tokenController.put = async (req, res) => {
  ...
  const token = await db.read("tokens", id);
  if (!token) {
    return res.json({ error: "The specified token does not exist" }, 404);
  }
  ...
  await db.update("tokens", id, token)
    .catch(e => res.json({ error: `Could not update the token's expiration date` }, 500));
  return res.json({}, 200);
};
...
tokenController.delete = async (req, res) => {
  ...
  const token = await db.read("tokens", id);
  if (!token) {
    return res.json({ error: "Could not find the specified token" }, 404);
  }
