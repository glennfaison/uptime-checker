...
function Srvr(protocol = http, options = {}) {
  ...
  server.route = (route = '') => ({
    get: (handler) => server.addRoute(route, 'get', handler),
    post: (handler) => server.addRoute(route, 'post', handler),
    put: (handler) => server.addRoute(route, 'put', handler),
    delete: (handler) => server.addRoute(route, 'delete', handler),
  })
